...
\begin{abstract}
% TODO: Bioinformatics sagt "A maximum of 150 words is recommended.", wir haben mehr als 250 !

% Große Menge an Microarray-Plattformtypen, aber wenig Verfahren zur integrierten Visualisierung

%\section{Motivation:}
%Today, a wide variety of different microarray platforms is available. Traditionally, microarrays were employed for the genome-wide analysis of differential gene expression in organisms, tissues, or cell types, monitored under varying experimental conditions. Nowadays, besides global mRNA profiling, their scope of application includes the interrogation of diverse genomic features, such as microRNAs, proteins, and promoters.
%
%To date, most available methods for the visualization of these datasets are focused on individual platforms and are not capable of integratively visualizing multiple microarray datasets from cross-platform studies. Furthermore, current pathway-based visualization tools do mostly not support genomic features which are not directly linked to protein-coding genes. Particularly, there is a demand for methods which can visualize regulatory RNAs (e.g., microRNAs) and epigenetic alterations (e.g., DNA methylation) in a pathway-centered manner.
%
%\section{Results:}
%We present a novel pathway-based visualization method that is especially suitable for the visualization of high-throughput datasets where multiple microarray platforms were employed for the analysis of diverse genomic features in the same set of biological samples. The proposed methodology includes concepts for linking DNA-methylation and microRNA expression datasets to canonical signaling and metabolic pathways by establishing connections to the protein-coding genes therein. Furthermore, we point out strategies for displaying data from multiple proteins and protein modifications corresponding to the same gene. Ultimately, we show how data from four distinct platform types (mRNA, miRNA, protein, and DNA methylation arrays) can be integratively visualized in the context of canonical pathways.


\section{Motivation:}
Traditionally, microarrays were employed for the genome-wide analysis of differential gene expression in samples, exposed to varying experimental conditions. Nowadays, besides global mRNA profiling, their scope of application has been extended to genomic features, such as microRNAs, proteins, and DNA methylation.

To date, most available methods for the visualization of these datasets are focused on individual platforms and are not capable of integratively visualizing multiple microarray datasets from cross-platform studies. Current pathway-based visualization tools do mostly not support genomic features which are not directly linked to protein-coding genes. Particularly, there is a demand for methods which can visualize regulatory RNAs (e.g., microRNAs) and epigenetic alterations (e.g., DNA methylation) in a pathway-centered manner.

\section{Results:}
We present a novel pathway-based visualization method that is especially suitable for the visualization of high-throughput datasets in which multiple microarray platforms were employed for the analysis of diverse genomic features in the same set of biological samples. The proposed methodology includes concepts for linking DNA-methylation and microRNA expression datasets to canonical signaling and metabolic pathways by establishing connections to the protein-coding genes therein. Furthermore, we point out strategies for displaying data from multiple proteins and protein modifications corresponding to the same gene. Ultimately, we show how data from four distinct platform types (mRNA, miRNA, protein, and DNA methylation arrays) can be integratively visualized in the context of canonical pathways.
...
% JE: Warum schreibst du nicht "www.cogsys.cs. ..."

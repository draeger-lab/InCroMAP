...
% Hoheisel2006 ist review f�r versch. MA technologien. DNAm, miRNA und protein ist drin!
...
from the same biological samples using different microarray platforms, which sample
...
In contrast to commonly used region-based visualization methods (e.g., the UCSC genome browser -- \citealp{UCSCBrowser}), we propose to visualize the microarray data in the context of
...
There are other tools, specialized in pathway analysis (e.g., Ingenuity), or in pathway visualization (e.g., Cytoscape -- \citealp{Cytoscape} or KEGG Atlas -- \citealp{KEGGAtlas}). Some even offer visualizing data in a pathway (e.g., KegArray -- \citealp{KEGG}, GenMAPP -- \citealp{GenMAPP} or MGV -- \citealp{Symons2011}). Most of those visualization tools offer simply changing the color of nodes. GenMAPP or MGV even habe capabilities to put multiple colors in a single node (e.g., for visualization of time-series experiments). MGV goes a step further and offers additional features to change the shape of nodes by putting XY plots or heatmaps inside nodes. But these tools are not able to handle data that have no direct reference to genes in a pathway (like microRNAs), and none offers solutions that are tailored for high-dimensional heterogeneous cross-platform datasets.


%But there is no method today that is tailored for high-dimensional heterogeneous cross-platform datasets.

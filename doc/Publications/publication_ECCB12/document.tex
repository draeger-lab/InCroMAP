...
To date, most available methods for the visualization of these datasets are focused on individual platforms and are not capable of integratively visualizing multiple microarray datasets from cross-platform studies. Futhermore, current pathway-based visualization tools do mostly not support genomic features which are not directly linked to protein-coding genes. Particularly, there is a demand for methods which can visualize regulatory RNAs (e.g., microRNAs) and epigenetic alterations (e.g., DNA methylation) in a pathway-centered manner. 

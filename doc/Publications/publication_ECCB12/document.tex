...
% Ich w√ºrde daher den folgenden Absatz streichen um den Leser nicht zu ausf√ºhrlich mit Details vollzulabern ;-)
%
...
% JE: W√ºrde den Absatz streichen, da er zu viele technische Details enth√§lt
...
from -2000 bps upstream of the transcription start site to 500 bps downstream. Normally, mean
log-ratios $r$ are used as summary value, which can be computed from the probe-level \emph{p}-values
$x_1,\dots,x_n$ using the formula $r=\frac{1}{n}\sum\limits_{i=1}^n\log_2(x_i)$. Alternatively, the
value of the maximum peak found in the promoter of a gene can be visualized. For peak detection,
...
user's guide of Nimblegen's SignalMap software \citep[see][]{NimblegenSignalMapUserGuide}. In addition to those sophisticated peak detection methods, one can also approximate peaks by using simple methods like picking the probe with maximum differential methylation (i.e., $\max|fold-change|$).

% Darstellung der DNAm Einzelwerte
The purpose of this visualization technique is giving researchers a hint if, and how many methylation changes are present in a gene promoter.
...
% Zusammenfassung: Transcriptomics-Daten
...
"Analyse der genetischen Loci von deregulierten miRNAs weiterf¸hren kann[]" eher f¸r regions-basierte verfahgen. PWs sind funktionielle/ regulatorische zusammenh‰nnge. => aber generell m¸ssen benachbarte loci nicht derselben genetischen regulation unterlegen und deshalb f¸r verbindung ist es besser regulatorische zusammenh‰nge zu verwenden
%
% TODO: Diskussionspunkt: Alternativ zu Abbildung auf Targets w√§re auch Abbildung auf Genlokus denkbar (Heidruns Vorschlag)
%
...
cover specific genomic regions of interest, as for instance promoters. The connection to protein
...
% Problematik bei Berechnung eines Gesamtwerts f√ºr DNAm
As visualizing the signal levels of all probes assigned to a certain gene in one pathway node would
clutter the picture and reduce the clarity of it, a method for the computation of a single
summary value is needed. For mRNA expression arrays, the summarization of probes corresponding to
the same transcript is usually achieved by the median polishing algorithm (TODO: cite RMA/GCRMA paper) which
effectively alleviates the influence of outlier probes. However, this algorithm is only applicable under the assumption
that ideally the probe intensities are expected to run evenly along transcripts.
...
DNA methylation peaks are quite likely not due to probe-specific effects, but rather reflect a local change
in the DNA methylation pattern, which may be biologically meaningful. Thus, one of the summarization methods, we
consider advisable is the detection and visualization of maximum peaks (TODO: cite Nimblegen's user's guide),
as it is sensitive to local changes in DNA methylation.

TODO: Probe-level specific effekte sollten abgeschw‰cht sein => Smoothing => dann max FC.

 Alternatively, we suggest to use the mean
probe-level fold-changes as a summary value, if global changes of DNA methylation in the proximal promoter region
are of peculiar interest. Nevertheless, both methods have obvious limitations. The maximum peaks method is definitely
less robust against probe-specific effects and the results may be inconclusive as a single positive peak may
suggest hypermethylation of a region, while the major part of the promoter is clearly hypomethylated.
On the other hand the mean fold-change summarization clearly suffers from the fact that localized differential methylation
is averaged out. Despite these limitations, in principle, we deem both methods suited for indicating
...
% TODO: Nested nodes (pics in nodes) diskutieren
...
% TODO f√ºr JE: Kapitel √ºberarbeiten
\section{Conclusion}

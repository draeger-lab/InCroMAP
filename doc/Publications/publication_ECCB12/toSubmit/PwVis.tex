...
\author[Clemens Wrzodek \textit{et~al}]{Clemens Wrzodek\,$^{1,*}$, Johannes Eichner\,$^{1}$ and Andreas Zell\,$^{1,}$\footnote{to whom correspondence should be addressed}}
...
methylation peaks in promoter regions. A bar from the middle of the grey box to the left represents
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% - History of microarrays, emergence of technologies other than mRNA
% o Enrichment, especially pathway-enrichment and showing a need for visualization
% - Visualization methods (region based vs. pathway-based)
...
various experimental conditions \citep{Schena1995, Golub1999}.
To date, a plethora of different microarray platforms are readily available. These
...
% Shift from individual genes analysis to groups of genes (GSEA) and need to visualize them (pw's)
Microarray datasets are typically not only inspected on the level of individual genes, but rather differential gene expression in the context of groups of functionally related genes. For instance, the overrepresentation of genes related to the same biological process or involved in the same pathway is investigated by using statistical tests (e.g., Fisher test or hypergeometric test) or gene set enrichment analysis (GSEA -- \citealp{Subramanian2005}).
Traditional overrepresentation analysis methods require fixed sets of up- or downregulated genes that are usually defined with fold-change and/or \emph{p}-value cutoffs. Modern GSEA-like methods, in contrast, do not require cutoffs to detect sets of functionally related genes showing a trend towards differential gene expression \citep{Markowetz2010}.
%
% NAchdem man ein pw-enrich gemacht hat, will man mehr details ¸ber die gene und deren st‰rke von hoch-runter haben. Am besten sucht man sich hierf¸r ein paar nette visualisierungstools.
%
As result of a pathway-enrichment, a list of significant pathways that contain genes with strong differential expression is returned. To obtain more details about this result (e.g., which genes are up- or downregulated within a pathway), suitable visualization methods are required.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Motivation fuer die Entwicklung des Tools
% Visualization of MA data (region vs pathway based)
Several tools exist for the visual inspection of datasets from individual platforms \citep[see][for
some examples]{Gehlenborg2010}.
%
As the first microarrays were mostly limited to mRNA transcripts, the vast majority of these visualization tools is still focused on mRNA datasets.
%
However, the current inventory of publicly available tools, that are
capable of integrating and jointly visualizing data from multiple microarray platforms, is still
very limited.
%
While for genomic features, which are not directly linked to genes (such as DNA methylation or SNP data) region-based visualization methods (e.g., the UCSC genome browser -- \citealp{UCSCBrowser}) are commonly used, data from gene-centered genomic features (mRNAs, proteins, \dots) is often visualized using network-based visualization methods (e.g., Cytoscape -- \citealp{Cytoscape} or KEGG Atlas -- \citealp{KEGGAtlas}). While the former approach may provide more detailed information about individual genes or genomic loci, conclusions about higher-order mechanisms can often be more easily drawn from a pathway-based view of the data. Thus, we propose to first focus on pathways relevant to the studied phenomenon, which can be identified using enrichment analysis methods. Subsequently, network hubs or key regulators can be inspected in more detail using complementary region-based visualization methods.
...
Relevant pathways can be deduced by a gene-set enrichment analysis or simply by manually choosing any pathway to visualize.
%
...
grey boxes left of the nodes represent the maximum DNA methylation fold-change peak. A bar to the
...
and all relations to edges. Compounds are depicted by small circular nodes, genes or proteins are rectangular shaped nodes and group nodes are used to illustrate complexes or families. The original layout, given in the KGML document, is used to position all nodes.
...
Among others, the InCroMAP application provides all of these summarization methods.

% Einf√§rbung der Knoten
...
% Problematik bei Berechnung eines Gesamtwerts fuerDNAm
DNA methylation (DNAm) microarrays are similar to tiling arrays and contain many probes that can be assigned to a single gene.
...
For DNA methylation in gene promoters, it is most important to know if there are methylation changes and if the promoter
is rather hyper- or hypomethylated. The methylation details can then be inspected manually later
(e.g., the InCroMAP application provides a detailed XY plot of the DNA methylation if one clicks on a pathway node, see Figure~\ref{fig:methdetails}).
%
% Darstellung von DNAm Daten
Therefore, the DNAm status, observed in the proximal promoter of a gene, is summarized and graphically
represented by adding boxes to the left side of the pathway nodes. These boxes are drawn on a white
background and contain a grey bar that stretches from the middle to the left, in the case of hypomethylation and from the middle to the right, to indicate hypermethylation. Instead of discriminating between hyper- and hypomethylation, one can simply visualize the amount of differential methylation. In this case, the grey bar would stretch from the left border of the box to the right border.

% Berechnung des DNAm Gesamtwerts
The length of the grey bar is proportional to a summary value reflecting the DNAm status of the
promoter of a certain gene. This value is by default computed from the probes in a region ranging
from -2000\,bps upstream of the transcription start site to 500\,bps downstream.
%
%
For mRNA expression arrays, the summarization of probes corresponding to
...
This assumption does not hold true for the DNA methylation of promoter regions, as the probe intensities are
not expected to be evenly distributed along the region. Furthermore, outlier probes, corresponding to
DNAm peaks, are mostly not due to probe-specific effects, but rather reflect a local change
in the DNAm pattern, which may be biologically meaningful.
A method to overcome this issue is the detection and visualization of maximum peaks within the DNAm data.
%
For peak detection, Nimblegen proposed two algorithms which are called `Windowed Threshold Detection' and
...
using simple methods such as picking the probe with maximum differential methylation (i.e., $\max|fold\text{-}change|$).
...
%
% Eingabeformat fÔøΩr DNAm Daten
...
in the proximal promoter of \emph{Egfr}. The grey DNA methylation bar next to the \emph{Egfr} gene in the pathway
plot stretches from the middle to the right of the box, which indicates strong hypermethylation. The length
of the grey bar corresponds to the height of the maximum peak of the red curve. This curve displays the probe-level
...
% miRNA discussion
MicroRNAs are known to have a regulatory function by binding to complementary mRNA transcripts \citep{Bartel2004}. These interactions are documented in public databases, which we use to extract interactions between miRNAs and their target mRNAs. Using this information, one can add the miRNAs to the pathways and connect them to the existing nodes by inserting edges to the corresponding targets.
...
%
% Problematik bei Berechnung eines Gesamtwerts fuerDNAm
% We need to summarize data, difference to mRNA summarization, Why not taking the mean.
%
%clutter picture, summary mit local changes (also kein mean) => maximum peaks.
However, the visualization of every probe would vastly reduce the clarity of the visualization and therefore, a summary method is required. For DNA methylation changes, calculating a mean is often inappropriate, because small peaks in a promoter region can already have a large influence on gene regulation. One of the summarization methods we consider advisable is the detection and visualization of maximum peaks as it is sensitive to local changes in DNA methylation.
%
...
% Beispiel f√ºr Visualisierung aller Plattform-Typen in einem Pathway
An example for a KEGG pathway with visualized mRNA, miRNA, protein and DNA methylation data can be
seen in Figure~\ref{fig:cellcycle}. The visualization of multiple heterogenous datatypes cannot be
...
visualization of DNA methylation and miRNA datasets is a feature that cannot be found in other
...
% JE: Sch√∂nes Beispiel. Habe noch die DNAm vor die miRNA gesetzt um alle Plattformen in der Kausalkette unterzubringen.
